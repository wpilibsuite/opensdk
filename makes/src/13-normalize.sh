...
if [ "${TARGET_DISTRO}" = "systemcore" ]; then
    ...
    mv "usr/include/c++/${V_GCC/.*/}/aarch64-buildroot-linux-gnu/bits"/* "usr/include/c++/14/bits/" || die "Couldn't move C++ bits/ headers"
    mv "usr/include/c++/${V_GCC/.*/}/aarch64-buildroot-linux-gnu/ext"/* "usr/include/c++/14/ext/" || die "Couldn't move C++ ext/ headers"
    ...
    # (3 times). We will choose to move the majority of libraries to
